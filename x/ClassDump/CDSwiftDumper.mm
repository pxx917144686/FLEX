...
static const NSUInteger kCDSwiftIndexMaxLines = 2000;

static void CDWriteSwiftIndex(NSString *rootDir, NSArray<NSString *> *sampleLines,
                              NSUInteger totalCount, NSMutableArray<NSString *> *written) {
    ...
    for (NSString *line in sampleLines) {
        [s appendFormat:@"%@\n", line];
    }
...
+ (NSArray<NSString *> *)dumpSwiftInterfacesAtRootDir:(NSString *)rootDir
    ...
    for (uint32_t i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            for (NSString *line in lines) {
                ...
                allLinesCount++;
                if (allLines.count < kCDSwiftIndexMaxLines) [allLines addObject:line];
    ...
    CDWriteSwiftIndex(rootDir, allLines, allLinesCount, written);
